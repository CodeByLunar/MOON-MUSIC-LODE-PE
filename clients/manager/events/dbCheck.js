      ...
      for (const key of keys) {
        ...
        if (Date.now() < expiryTimestamp && Date.now() >= reminderTime) {
          client.users
            ...
            .then(async (user) => {
              let desc =
                `${emoji.warn} Your premium for Moon Music is expiring tomorrow !\n` +
                `Please visit **[Support Server](https://dsc.gg/sideshop)** for further details or use Pookie's command \`+buy\` to renew your subscription`;
