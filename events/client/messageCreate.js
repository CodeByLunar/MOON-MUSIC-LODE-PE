...
module.exports = {
  ...
  run: async (client, message) => {
    ...
    let [pfxu, pfxg, ignoredChnl, blacklistUser, blacklistServer, owner, admin] =
      await Promise.all([
        ...
        client.db.blacklist.get(`sbl_${message.guild.id}`),
        client.owners.find((x) => x === message.author.id),
        client.admins.find((x) => x === message.author.id),
      ]);

    const isCommand = message.content.startsWith(client.prefix) || client.commands.has(message.content.toLowerCase());
    if (blacklistServer && isCommand) {
      return await message.reply({
        embeds: [
          new client.embed().desc(
            `${client.emoji.no} **This server is blacklisted from using the bot.**`
          ),
        ],
      });
    }
    ...
    if (message.content.includes("<@1243892201252196420>")) {
      await message.react("<a:zzkittySleeping:1330118142235512903>");
      await message.react("<:code:1330117890409631754>");
    }
    ...
    if (premiumUser && !message.content.startsWith(client.prefix)) {
      prefix =
        (message.content.toLowerCase().startsWith(pfxu?.toLowerCase()) ? pfxu : "") ||
        (message.content.toLowerCase().startsWith(pfxg?.toLowerCase()) ? pfxg : "");
    }
    ...
    if (command.owner && !command.admin && !owner) {
      return await message.reply({
        embeds: [
          new client.embed().desc(
