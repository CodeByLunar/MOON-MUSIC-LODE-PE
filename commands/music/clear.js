...
module.exports = {
  ...
  execute: async (client, message, args, emoji) => {
    ...
    let emb = new client.embed().desc(
      `<a:music:1262124876685381704> **Choose an option to clear :**`,
    );
    let m = await message
    ...
    const collector = m?.createMessageComponentCollector({
      filter: (i) => {
        if (i.user.id === message.author.id) return true;
          ...
          i.reply({
            ephemeral: true,
            content: `Only **${message.author.tag}** can use this command`,
          })
            .catch((err) => {
              int.deferUpdate();
            })
        ...
        await m
          .edit({
            embeds: [
              new client.embed()
                .desc(
                  `<a:music:1262124876685381704> **Timed out ! (Choose an option to clear)**`,
                )
                .setFooter({
