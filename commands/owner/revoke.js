module.exports = {
  ...
  description: "Remove static (blacklist user/server or remove premium)",
  args: true,
  vote: false,
  new: false,
  admin: true,
  ...
  execute: async (client, message, args, emoji) => {
    ...
    switch (static) {
      ...
      case "premium":
            ...
            embeds: [
              new client.embed().desc(
                `${emoji.no} **Operation unsuccessful**\n` +
                  `${emoji.bell} <@${id}> / \`${id}\` does not have the static: \`${static}\``
              ),
            ],
          });
        }
        ...
        await client.db[db].delete(key);

        await message.reply({
          embeds: [
            new client.embed().desc(
              `${emoji.yes} **Operation successful**\n` +
                `${emoji.off} Removed the static \`${static}\` from <@${id}> / \`${id}\``
            ),
          ],
        });
